...
abstract class MigrateUiParagraphsTestBase extends MigrateUpgradeTestBase {
  ...
  protected function setParagraphsMigrationDerivedByParent(bool $derive_by_parent) {
    $current_method = Settings::get('paragraphs_migration_derivation_method', FALSE);

    if ($current_method !== $derive_by_parent) {
      $settings['settings']['paragraphs_migration_derivation_method'] = (object) [
        'value' => $derive_by_parent,
        'required' => TRUE,
      ];
      $this->writeSettings($settings);
    }
